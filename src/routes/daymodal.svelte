<script lang="ts">
    ...
    const isYesterday = (date: Date) => date.toDateString() === new Date(new Date().setDate(new Date().getDate() - 1)).toDateString();
    const isToday = (date: Date) => date.toDateString() === new Date().toDateString();
    const isTomorrow = (date: Date) => date.toDateString() === new Date(new Date().setDate(new Date().getDate() + 1)).toDateString();
    const isFutureDay = (date: Date) => date > new Date();

    const daysOut = (date: Date) => Math.round(Math.abs(new Date().getTime() - selectedDate.getTime()) / (1000 * 60 * 60 * 24)) + 1

    let selectedDate: Date, formattedDate: string | null = null;
    selectedDay.subscribe((value: any) => {
        ...
        formattedDate = formatter.format(new Date(new Date().getFullYear(), value.month, value.date + 1));
    });
    ...
    function click(e: MouseEvent) {
        if (
            ...
            e.target === closeButton || closeButton.contains(e.target as HTMLElement)
        ) {
            ...
            <div class="flex flex-col gap-4">
                ...
                {#if selectedDate}
                    ...
                    {@const text = 
                        ...
                        isFutureDay(selectedDate) ? `In ${daysOut(selectedDate)} days` :
